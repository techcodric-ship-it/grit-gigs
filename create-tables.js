// ================================================================
// Grit&Gigs — one-time database setup
...
CREATE TABLE IF NOT EXISTS users (
  ...
  profile_photo TEXT, bio TEXT, city VARCHAR(100),
  ...
  plan_activated_at TIMESTAMP, plan_expires_at TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  link_url TEXT, is_read BOOLEAN NOT NULL DEFAULT false,
  created_at TIMESTAMP NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS withdrawal_requests (
  ...
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  amount REAL NOT NULL, bank_name TEXT NOT NULL,
  account_number TEXT NOT NULL, ifsc_code TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS transactions (
  ...
  payment_method VARCHAR(50), gateway_txn_id TEXT, description TEXT,
  created_at TIMESTAMP NOT NULL DEFAULT now(),
  updated_at TIMESTAMP NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS services (
  ...
  title TEXT NOT NULL, category TEXT NOT NULL, subcategory TEXT,
  description TEXT NOT NULL,
  images TEXT[] NOT NULL DEFAULT '{}', tags TEXT[] NOT NULL DEFAULT '{}',
  status service_status NOT NULL DEFAULT 'ACTIVE',
  view_count INTEGER NOT NULL DEFAULT 0, order_count INTEGER NOT NULL DEFAULT 0,
  rating_avg REAL NOT NULL DEFAULT 0, review_count INTEGER NOT NULL DEFAULT 0,
  created_at TIMESTAMP NOT NULL DEFAULT now(),
  updated_at TIMESTAMP NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS service_packages (
  ...
  package_type TEXT NOT NULL, price_credits REAL NOT NULL,
  description TEXT NOT NULL, delivery_days INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  price_credits REAL NOT NULL, requirements JSONB,
...
CREATE TABLE IF NOT EXISTS order_deliveries (
  ...
  files TEXT[] NOT NULL DEFAULT '{}', message TEXT,
...
CREATE TABLE IF NOT EXISTS reviews (
  ...
  barter_match_id UUID, rating INTEGER NOT NULL,
  review_text TEXT, seller_response TEXT,
...
CREATE TABLE IF NOT EXISTS projects (
  ...
  skills TEXT, budget_min INTEGER, budget_max INTEGER,
  deadline TIMESTAMP,
  image_url TEXT,
...
CREATE TABLE IF NOT EXISTS project_bids (
  ...
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  amount INTEGER NOT NULL, proposal TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS conversations (
  ...
  user2_id UUID NOT NULL REFERENCES users(id),
  order_id UUID UNIQUE REFERENCES orders(id),
  match_id UUID UNIQUE REFERENCES barter_matches(id),
...
async function run() {
  ...
  try {
    ...
    console.log('👉 Then try posting a project — it will work!\n');
  } catch (err) {
    console.error('\n❌ Error:', err.message);

...
router.get("/orders/:id", authenticate, async (req, res): Promise<void> => {
  ...
  const [buyer] = await db    .select({ id: usersTable.id, firstName: usersTable.firstName, lastName: usersTable.lastName, profilePhoto: usersTable.profilePhoto, city: usersTable.city, kycVerified: usersTable.kycVerified, isActive: usersTable.isActive }).from(usersTable).where(eq(usersTable.id, order.buyerId));

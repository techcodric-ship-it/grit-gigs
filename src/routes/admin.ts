...
// ── Pending UPI payments (admin) ──
router.get("/admin/upi-pending", async (_req: Request, res: Response) => {
  try {
    const rows = await db
      .select({
      ...
      .where(and(eq(transactionsTable.paymentMethod, "upi_manual"), eq(transactionsTable.status, "PENDING")))
      .orderBy(desc(transactionsTable.createdAt));
    res.json({ success: true, data: rows });
  } catch (err) {
    res.status(500).json({ success: false, message: "Failed to fetch pending UPI payments" });
  }
});

router.post("/admin/upi-confirm/:txnId", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (txn.status !== "PENDING") { res.status(400).json({ success: false, message: "Already processed" }); return; }

    const amtInr = txn.amount;
    const walletResult = await db.execute(
      sql`UPDATE ${freelanceWalletsTable} SET balance = balance + ${amtInr}, updated_at = NOW() WHERE ${freelanceWalletsTable.userId} = ${txn.userId}`
    );
    ...
    await db.insert(notificationsTable).values({
      ...
      message: "Your manual UPI payment has been confirmed and wallet credited.",

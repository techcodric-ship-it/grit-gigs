...
window.openServiceModal = async function(serviceId, title, price, profilePhoto, sellerName) {
  ...
  modal.querySelector('.modal').innerHTML = `
    ...
    <div style="display:flex;gap:10px;align-items:center;margin-bottom:12px;">
      ${sellerAv}
      <div><div style="font-weight:600;font-size:0.9rem;cursor:pointer;" onclick="closeModal('serviceModal');location.href='${profileUrl}'">${sellerName}</div><div style="font-size:0.75rem;color:var(--text-muted);">${s.seller?.city||'India'} · Rep ${s.seller?.reputationScore||0}</div></div>
